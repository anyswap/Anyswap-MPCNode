...
func sign_ec(msgprex string, txhash string, save string, dcrmpkx *big.Int, dcrmpky *big.Int, keytype string, ch chan interface{}) string {
	...
	///////
	bak_sig := SignEc2(msgprex, save, txhash, keytype, dcrmpkx, dcrmpky, ch, id)
	return bak_sig

}

func SignEc2(msgprex string, save string, message string, keytype string, pkx *big.Int, pky *big.Int, ch chan interface{}, id int) string {
	...
	skU1, w1 := MapPrivKeyShare2(keytype, w, idSign, mm[0])
	if skU1 == nil || w1 == nil {
		return ""
	}
	...
	ukc, ukc2, ukc3 := DECDSASignPaillierEncrypt2(keytype, save, w, idSign, u1K, ch)
	if ukc == nil || ukc2 == nil || ukc3 == nil {
		return ""
	}
	common.Info("===================sign,paillier encrypt finish===========================")

	zk1proof, zkfactproof := DECDSASignRoundTwo2(msgprex, keytype, save, w, idSign, ch, u1K, ukc2, ukc3)
	if zk1proof == nil || zkfactproof == nil {
		return ""
	}
	common.Info("===================sign,round two finish===========================")

	if DECDSASignRoundThree2(msgprex, keytype, save, w, idSign, ch, ukc) == false {
		return ""
	}
	common.Info("===================sign,round three finish===========================")

	if DECDSASignVerifyZKNtilde2(msgprex, keytype, save, w, idSign, ch, ukc, ukc3, zk1proof, zkfactproof) == false {
		return ""
	}
	...
	mkg, mkg_mtazk2, mkw, mkw_mtazk2, status := DECDSASignRoundFour2(msgprex, keytype, save, w, idSign, ukc, ukc3, zkfactproof, u1Gamma, w1, betaU1Star, vU1Star, ch)
	if status != true {
		return ""
	}
	common.Info("===================sign,round four finish===========================")

	if DECDSASignVerifyZKGammaW2(keytype, save, w, idSign, ukc, ukc3, zkfactproof, mkg, mkg_mtazk2, mkw, mkw_mtazk2, ch) != true {
		return ""
	}
	common.Info("===================sign,verify zk gamma/w finish===========================")

	u1PaillierSk := GetSelfPrivKey2(keytype, idSign, w, save, ch)
	if u1PaillierSk == nil {
		return ""
	}
	common.Info("===================sign,get self privkey finish===========================")

	alpha1 := DecryptCkGamma2(keytype, idSign, w, u1PaillierSk, mkg, ch)
	if alpha1 == nil {
		return ""
	}
	common.Info("===================sign,decrypt paillier(k)XGamma finish===========================")

	uu1 := DecryptCkW2(keytype, idSign, w, u1PaillierSk, mkw, ch)
	if uu1 == nil {
		return ""
	}
	...
	deltaSum := DECDSASignRoundFive2(msgprex, keytype, delta1, idSign, w, ch)
	if deltaSum == nil {
		return ""
	}
	...
	ug := DECDSASignVerifyCommitment2(keytype, w, idSign, commitU1GammaG, u1GammaZKProof, ch)
	if ug == nil {
		return ""
	}
	common.Info("===================sign,verify commitment finish===========================")

	r, deltaGammaGy := Calc_r2(keytype, w, idSign, ug, deltaSum, ch)
	...
	commitbigcom, BigVx, BigVy := DECDSASignVerifyBigVAB2(keytype, w, commitbigvabs, zkabproofs, commitBigVAB1, u1zkABProof, idSign, r, deltaGammaGy, ch)
	if commitbigcom == nil || BigVx == nil || BigVy == nil {
		return ""
	}
	common.Info("===================sign,verify BigVAB finish===========================")

	commitbiguts, commitBigUT1 := DECDSASignRoundNine2(msgprex, keytype, w, idSign, mMtA, r, pkx, pky, BigVx, BigVy, rho1, commitbigcom, l1, ch)
	...
	if DECDSASignVerifyBigUTCommitment2(keytype, commitbiguts, commitbigutd11s, commitBigUT1, w, idSign, ch, commitbigcom) != true {
		return ""
	}
	common.Info("===================sign,verify BigUT commitment finish===========================")

	ss1s := DECDSASignRoundEleven2(msgprex, keytype, w, idSign, ch, us1)
	if ss1s == nil {
		return ""
	}
	common.Info("===================sign,round eleven finish===========================")

	s := Calc_s2(keytype, w, idSign, ss1s, ch)
	if s == nil {
		return ""
	}
	...
	recid := DECDSA_Sign_Calc_v(r, deltaGammaGy, pkx, pky, signature.GetR(), signature.GetS(), hashBytes, false)
...
func MapPrivKeyShare2(keytype string, w *RpcReqWorker, idSign sortableIDSSlice, privshare string) (*big.Int, *big.Int) {
	if keytype == "" || w == nil || len(idSign) == 0 || privshare == "" {
	    return nil, nil
	}
	...
	lambda1 := big.NewInt(1)
	for _, uid := range idSign {
		enodes := GetEnodesByUid2(uid, keytype, w.groupid)
		if IsCurNode(enodes, cur_enode) {
			self = uid
			break
		}
...
func DECDSASignPaillierEncrypt2(keytype string, save string, w *RpcReqWorker, idSign sortableIDSSlice, u1K *big.Int, ch chan interface{}) (map[string]*big.Int, map[string]*big.Int, map[string]*ec2.PublicKey) {
	if keytype == "" || w == nil || len(idSign) == 0 || u1K == nil {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error")}
		ch <- res
		return nil, nil, nil
	}
	...
	for _, id := range idSign {
		enodes := GetEnodesByUid2(id, keytype, w.groupid)
		en := strings.Split(string(enodes[8:]), "@")
		if IsCurNode(enodes, cur_enode) {
			u1PaillierPk := GetPaillierPk(save, GetRealByUid2(keytype,w,id))
			if u1PaillierPk == nil {
				res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("get save paillier pk fail")}
				ch <- res
				return nil, nil, nil
...
func DECDSASignRoundTwo2(msgprex string, keytype string, save string, w *RpcReqWorker, idSign sortableIDSSlice, ch chan interface{}, u1K *big.Int, ukc2 map[string]*big.Int, ukc3 map[string]*ec2.PublicKey) (map[string]*ec2.MtAZK1Proof_nhh, map[string]*ec2.NtildeH1H2) {
	if msgprex == "" || keytype == "" || save == "" || w == nil || len(idSign) == 0 || u1K == nil || len(ukc2) == 0 || len(ukc3) == 0 {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error")}
		ch <- res
		return nil, nil
	}
	...
	for _, id := range idSign {
		...
		u1zkFactProof := GetZkFactProof(save, GetRealByUid2(keytype,w,id), w.NodeCnt)
		if u1zkFactProof == nil {
			fmt.Println("=================Sign_ec2,u1zkFactProof is nil. Nonce =%s=====================", msgprex)
			res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("get ntildeh1h2 fail")}
			ch <- res
		...
		if len(en) == 0 || en[0] == "" {
			fmt.Println("=================Sign_ec2,get enode error,Nonce =%s,enodes = %s,uid = %v,keytype = %s,groupid = %s =====================", msgprex, enodes, id, keytype, w.groupid)
			res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("get ntildeh1h2 fail")}
			ch <- res
			return nil, nil
		}
...
func DECDSASignRoundThree2(msgprex string, keytype string, save string, w *RpcReqWorker, idSign sortableIDSSlice, ch chan interface{}, ukc map[string]*big.Int) bool {
	if msgprex == "" || keytype == "" || save == "" || w == nil || len(idSign) == 0 || len(ukc) == 0 {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error")}
		ch <- res
		return false
	}
...
func DECDSASignVerifyZKNtilde2(msgprex string, keytype string, save string, w *RpcReqWorker, idSign sortableIDSSlice, ch chan interface{}, ukc map[string]*big.Int, ukc3 map[string]*ec2.PublicKey, zk1proof map[string]*ec2.MtAZK1Proof_nhh, zkfactproof map[string]*ec2.NtildeH1H2) bool {
	if msgprex == "" || keytype == "" || save == "" || w == nil || len(idSign) == 0 || len(ukc) == 0 || len(ukc3) == 0 || len(zk1proof) == 0 || len(zkfactproof) == 0 {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error")}
		ch <- res
		return false
	}
...
func DECDSASignRoundFour2(msgprex string, keytype string, save string, w *RpcReqWorker, idSign sortableIDSSlice, ukc map[string]*big.Int, ukc3 map[string]*ec2.PublicKey, zkfactproof map[string]*ec2.NtildeH1H2, u1Gamma *big.Int, w1 *big.Int, betaU1Star []*big.Int, vU1Star []*big.Int, ch chan interface{}) (map[string]*big.Int, map[string]*ec2.MtAZK2Proof_nhh, map[string]*big.Int, map[string]*ec2.MtAZK3Proof_nhh, bool) {
	if msgprex == "" || keytype == "" || save == "" || w == nil || len(idSign) == 0 || len(ukc) == 0 || len(ukc3) == 0 || len(zkfactproof) == 0 || len(betaU1Star) == 0 || len(vU1Star) == 0 || u1Gamma == nil || w1 == nil {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error")}
		ch <- res
		return nil, nil, nil, nil, false
	}
	...
	for k, id := range idSign {
		...
		u2PaillierPk := GetPaillierPk(save, GetRealByUid2(keytype,w,id))
		if u2PaillierPk == nil {
			res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("get paillier pk fail")}
			ch <- res
			return nil, nil, nil, nil, false
...
func DECDSASignVerifyZKGammaW2(keytype string, save string, w *RpcReqWorker, idSign sortableIDSSlice, ukc map[string]*big.Int, ukc3 map[string]*ec2.PublicKey, zkfactproof map[string]*ec2.NtildeH1H2, mkg map[string]*big.Int, mkg_mtazk2 map[string]*ec2.MtAZK2Proof_nhh, mkw map[string]*big.Int, mkw_mtazk2 map[string]*ec2.MtAZK3Proof_nhh, ch chan interface{}) bool {
	if keytype == "" || save == "" || w == nil || len(idSign) == 0 || len(ukc) == 0 || len(ukc3) == 0 || len(zkfactproof) == 0 || len(mkg) == 0 || len(mkw) == 0 || len(mkg_mtazk2) == 0 || len(mkw_mtazk2) == 0 {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error")}
		ch <- res
		return false
	}
...
func GetSelfPrivKey2(keytype string, idSign sortableIDSSlice, w *RpcReqWorker, save string, ch chan interface{}) *ec2.PrivateKey {
	if keytype == "" || len(idSign) == 0 || w == nil || save == "" {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error")}
		ch <- res
		return nil
	}
	...
	u1PaillierSk := GetPaillierSk(save,GetRealByUid2(keytype,w,uid)) //get self privkey
	if u1PaillierSk == nil {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("get sk fail.")}
		ch <- res
		return nil
...
func DecryptCkGamma2(keytype string, idSign sortableIDSSlice, w *RpcReqWorker, u1PaillierSk *ec2.PrivateKey, mkg map[string]*big.Int, ch chan interface{}) []*big.Int {
	if keytype == "" || len(idSign) == 0 || w == nil || u1PaillierSk == nil || len(mkg) == 0 {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error")}
		ch <- res
		return nil
	}
...
func DecryptCkW2(keytype string, idSign sortableIDSSlice, w *RpcReqWorker, u1PaillierSk *ec2.PrivateKey, mkw map[string]*big.Int, ch chan interface{}) []*big.Int {
	if keytype == "" || len(idSign) == 0 || w == nil || u1PaillierSk == nil || len(mkw) == 0 {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error")}
		ch <- res
		return nil
	}
...
func DECDSASignRoundFive2(msgprex string, keytype string, delta1 *big.Int, idSign sortableIDSSlice, w *RpcReqWorker, ch chan interface{}) *big.Int {
	if keytype == "" || len(idSign) == 0 || w == nil || msgprex == "" || delta1 == nil {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error")}
		ch <- res
		return nil
	}
...
func DECDSASignVerifyCommitment2(keytype string, w *RpcReqWorker, idSign sortableIDSSlice, commitU1GammaG *ec2.Commitment, u1GammaZKProof *ec2.ZkUProof, ch chan interface{}) map[string][]*big.Int {
	if keytype == "" || w == nil || len(idSign) == 0 || commitU1GammaG == nil || u1GammaZKProof == nil {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error")}
		ch <- res
		return nil
	}
...
func Calc_r2(keytype string, w *RpcReqWorker, idSign sortableIDSSlice, ug map[string][]*big.Int, deltaSum *big.Int, ch chan interface{}) (*big.Int, *big.Int) {
	if keytype == "" || w == nil || len(idSign) == 0 || len(ug) == 0 {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error")}
		ch <- res
		return nil, nil
	}
	...
	var GammaGSumy *big.Int
	for _, id := range idSign {
		enodes := GetEnodesByUid(id, keytype, w.groupid)
		////////bug
		if len(enodes) < 9 {
			res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("get enodes error")}
			ch <- res
...
func DECDSASignVerifyBigVAB2(keytype string, w *RpcReqWorker, commitbigvabs []string, zkabproofs []string, commitBigVAB1 *ec2.Commitment, u1zkABProof *ec2.ZkABProof, idSign sortableIDSSlice, r *big.Int, deltaGammaGy *big.Int, ch chan interface{}) (map[string]*ec2.Commitment, *big.Int, *big.Int) {
	if len(commitbigvabs) == 0 || len(zkabproofs) == 0 || commitBigVAB1 == nil || u1zkABProof == nil || keytype == "" || w == nil || len(idSign) == 0 || r == nil || deltaGammaGy == nil {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error.")}
		ch <- res
		return nil, nil, nil
	}
...
func DECDSASignRoundNine2(msgprex string, keytype string, w *RpcReqWorker, idSign sortableIDSSlice, mMtA *big.Int, r *big.Int, pkx *big.Int, pky *big.Int, BigVx *big.Int, BigVy *big.Int, rho1 *big.Int, commitbigcom map[string]*ec2.Commitment, l1 *big.Int, ch chan interface{}) ([]string, *ec2.Commitment) {
	if len(idSign) == 0 || len(commitbigcom) == 0 || msgprex == "" || w == nil || keytype == "" || mMtA == nil || r == nil || pkx == nil || pky == nil || l1 == nil || rho1 == nil {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error.")}
		ch <- res
		return nil, nil
	}
...
func DECDSASignVerifyBigUTCommitment2(keytype string, commitbiguts []string, commitbigutd11s []string, commitBigUT1 *ec2.Commitment, w *RpcReqWorker, idSign sortableIDSSlice, ch chan interface{}, commitbigcom map[string]*ec2.Commitment) bool {
	if keytype == "" || len(commitbiguts) == 0 || len(commitbigutd11s) == 0 || commitBigUT1 == nil || w == nil || len(idSign) == 0 || commitbigcom == nil {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error.")}
		ch <- res
		return false
	}
...
func DECDSASignRoundEleven2(msgprex string, keytype string, w *RpcReqWorker, idSign sortableIDSSlice, ch chan interface{}, us1 *big.Int) map[string]*big.Int {
	if keytype == "" || msgprex == "" || w == nil || len(idSign) == 0 || us1 == nil {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error.")}
		ch <- res
		return nil
	}
...
func Calc_s2(keytype string, w *RpcReqWorker, idSign sortableIDSSlice, ss1s map[string]*big.Int, ch chan interface{}) *big.Int {
	if keytype == "" || len(idSign) == 0 || w == nil || len(ss1s) == 0 {
		res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("param error.")}
		ch <- res
		return nil
	}
...
func sign_ed(msgprex string,txhash string,save string, pk string, keytype string, ch chan interface{}) string {
	...
	bak_sig := Sign_ed2(msgprex, save, txhash, keytype, pk, ch, id)
	return bak_sig
}

func Sign_ed2(msgprex string, save string, message string, keytype string, pk string, ch chan interface{}, id int) string {
	defer func() {
		if e := recover(); e != nil {
	...
	for _, id := range ids {
		enodes := GetEnodesByUid2(id, keytype, GroupId)
		en := strings.Split(string(enodes[8:]), "@")
...
func GetEnodesByUid2(uid *big.Int, keytype string, groupid string) string {
	_, nodes := GetGroup(groupid)
	others := strings.Split(nodes, SepSg)
	for _, v := range others {
		node2 := ParseNode(v) //bug??
		id := DoubleHash2(node2, keytype)
		if id.Cmp(uid) == 0 {
			return v
		}
	}
