...
func GetCurNodeSignInfo(geter_acc string) ([]*SignCurNodeInfo, string, error) {
	...
	for iter.Next() {
	    ...
	    //fmt.Printf("=========================GetCurNodeSignInfo,start get pubkey data, key = %v============================\n",string(key2))
	    ...
	    vv2,ok := da.(*AcceptSignData)
	    if vv2 == nil || !ok {
		fmt.Printf("=========================GetCurNodeSignInfo, get AcceptSignData fail, key = %v, data = %v============================\n",string(key2),da)
		continue	
	    }

	    wg.Add(1)
	    go func(key string,vv *AcceptSignData) {
		defer wg.Done()

		if vv.Deal == "true" || vv.Status == "Success" {
		    //fmt.Printf("=========================GetCurNodeSignInfo, the sign info has done, key = %v============================\n",key)
		    return
		}

		if vv.Status != "Pending" {
		    //common.Debug("================GetCurNodeSignInfo, the sign info status is not pending======================","vv",vv,"vv.Deal",vv.Deal,"vv.Status",vv.Status,"key",key)
		    return
		}

		if !CheckAccept(vv.PubKey,vv.Mode,geter_acc) {
			fmt.Printf("=========================GetCurNodeSignInfo, check accept fail, key = %v============================\n",key)
			return
		}
...
func sign_ec(msgprex string, txhash []string, save string, sku1 *big.Int, dcrmpkx *big.Int, dcrmpky *big.Int, keytype string, pickdata []*PickHashData,ch chan interface{}) string {
	...
	for _,v := range tmp {
		...
		common.Info("======================sign_ec=================","unsign txhash",vv,"msgprex",msgprex,"key",key,"pick key",pick.Key)
		...
		val,err := json.Marshal(m)
		if err != nil {
		    common.Info("======================sign_ec, encode error==================","unsign txhash",vv,"msgprex",msgprex,"key",key,"pick key",pick.Key,"err",err)
		    return 
		}
