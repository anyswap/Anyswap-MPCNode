...
type RPCReqWorker struct {
	...
	msg_zku      *list.List
	msg_checkpresigndatastatus      *list.List
	...
	bzkfact           chan bool
	bzku              chan bool
	bcheckpresigndatastatus              chan bool
...
func NewRPCReqWorker(workerPool chan chan RPCReq) *RPCReqWorker {
	return &RPCReqWorker{
		...
		msg_zkfact:                list.New(),
		msg_zku:                   list.New(),
		msg_checkpresigndatastatus:                   list.New(),
		...
		bcheckpresigndatastatus:              make(chan bool, 1),
...
func (w *RPCReqWorker) Clear() {
	...
	for e := w.msg_checkpresigndatastatus.Front(); e != nil; e = next {
		next = e.Next()
		w.msg_checkpresigndatastatus.Remove(e)
	}
	...
	if len(w.bcheckpresigndatastatus) == 1 {
		<-w.bcheckpresigndatastatus
	}
