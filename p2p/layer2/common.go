...
func HandlePeer(peer *p2p.Peer, rw p2p.MsgReadWriter) error {
	...
	for {
		...
		if err != nil {
			...
			fmt.Printf("==== handle() ====, %v, rw.ReadMsg err: %v\n", peer.ID(), err)
			return err
		}
		switch msg.Code {
		case peerMsgCode:
